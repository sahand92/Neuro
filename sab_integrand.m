...
subplot(1,2,1)
freq=w/(2*pi);
plot(freq,Iee,freq,Iei,freq,Ies,freq,Iie,freq,Iii,freq,Iis,freq,Ire,freq,Irs,'--',freq,Ise,'--',freq,Isr,'--')
legend('I_{ee}','I_{ei}','I_{es}','I_{ie}','I_{ii}','I_{is}','I_{re}','I_{rs}','I_{se}','I_{sr}')
hold on 
% plot(w/(2*pi),Iei)
% hold on
% plot(w/(2*pi),Ies)
...
ylabel('P(s^{-1})')
hold on
plot(alpha_freq*ones(10),linspace(10^-8,10^-3,10),'--','color','green')
hold on
plot(2*alpha_freq*ones(10),linspace(10^-8,10^-3,10),'--','color','green')
hold on
plot(3*alpha_freq*ones(10),linspace(10^-8,10^-3,10),'--','color','green')
text(alpha_freq+1,10^-8*1.1,'f_\alpha')
text(2*alpha_freq+1,10^-8*1.1,'2f_\alpha')
text(3*alpha_freq+1,10^-8*1.1,'3f_\alpha')
set(gca,'XLim',[1 50],'YLim',[10^-8 10^-3]);
%END****************************
